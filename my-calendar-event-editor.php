...
function mc_create_event_post( $data, $event_id ) {
	...
	if ( ! $post_id ) {
		...
		if ( isset( $post['location_preset'] ) && is_numeric( $post['location_preset'] ) && 0 !== (int) $post['location_preset'] ) {
...
/**
 * Globally remote transient caches for categories, locations, and events.
 */
function mc_clear_all_transients() {
	$mcdb = mc_is_remote_db();
	// Unqualified transients.
	delete_transient( 'mc_generated_category_styles' );
	delete_transient( 'mc_get_date_bounds' );
	// Category transients.
	$categories = mc_get_all_categories();
	foreach ( $categories as $category ) {
		delete_transient( 'mc_cat_' . $category->category_id );
	}
	// Location transients.
	$locations = mc_get_locations( 'all' );
	foreach ( $locations as $location ) {
		delete_transient( 'mc_location_' . $location->location_id );
	}
	// Event transients.
	$events = $mcdb->get_results( 'SELECT event_id FROM ' . my_calendar_table() ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	foreach ( $events as $event ) {
		delete_transient( 'mc_categories_' . $event->event_id );
		delete_transient( 'mc_first_event_cache_' . $event->event_id );
	}
}
